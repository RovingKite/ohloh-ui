...
CREATE FUNCTION admin_insert_cl_py_ages_stats_v2() RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
...
CREATE FUNCTION admin_insert_cl_py_by_month_stats_v2() RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
...
CREATE FUNCTION admin_insert_cl_total_stats_v2() RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
...
CREATE FUNCTION admin_insert_cl_visited_stats_v2() RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
...
CREATE FUNCTION admin_select_cl_visited_stats_v2(interval_span character varying DEFAULT NULL::character varying, registration_key character varying DEFAULT NULL::character varying) RETURNS jsonb
    LANGUAGE plpgsql
    AS $_$
...
CREATE INDEX index_jobs_on_priority ON jobs USING btree (priority);
...
CREATE INDEX index_jobs_on_status_type_wait_until ON jobs USING btree (status, type, (COALESCE(wait_until, '1980-01-01 00:00:00'::timestamp without time zone)));
...
CREATE INDEX index_subscriptions_client_relation_id ON subscriptions USING btree (code_location_id, registration_key_id, client_relation_id) WHERE (client_relation_id IS NOT NULL);
...
CREATE INDEX index_subscriptions_null_client_relation_id ON subscriptions USING btree (code_location_id, registration_key_id) WHERE (client_relation_id IS NULL);
...
CREATE INDEX index_subscriptions_on_code_location_id ON subscriptions USING btree (code_location_id);
...
ALTER TABLE ONLY subscriptions
    ADD CONSTRAINT fk_rails_481c653bad FOREIGN KEY (registration_key_id) REFERENCES registration_keys(id);
